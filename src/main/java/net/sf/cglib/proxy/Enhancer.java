...
/**
 * // 20201112 生成动态子类用于启用方法拦截
 * // 20201112 此类开始是JDK 1.3附带的JDK动态代理支持的替代, 但该类允许代理除了实现接口外的委托类
 * // 20201112 动态生成的子类会覆盖父类的non-final方法, 并具有回调, 回调到用户定义的拦截器实现
...
public class Enhancer extends AbstractClassGenerator
{
     ...
     */
    // 20201112 设置委托类, 如果提供的超类实际上是接口, 则将调用setInterfaces()进行父类设置, 其中非接口参数不能声明为final, 并且必须具有可访问的构造函数
    public void setSuperclass(Class superclass) {
        if (superclass != null && superclass.isInterface()) {
            // 20201112 如果接口实现, 则调用setInterfaces()进行父类设置
            setInterfaces(new Class[]{ superclass });
        } else if (superclass != null && superclass.equals(Object.class)) {
            // 20201112 如果是父类继承, 则影响ClassLoader的选择?
            // affects choice of ClassLoader
            this.superclass = null;
        } else {
            // 20201112 如果没有, 则设置父类为自己本身
     ...
     * @see #setCallback
     */
    // 20201112 校验后, 设置回调函数到Enhancer的成员变量中
